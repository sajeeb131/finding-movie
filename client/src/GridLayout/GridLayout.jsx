import React from "react";
import './styles.css'
import FirstGrid from "../first-grid/FirstGrid";
import SecondGrid from "../second-grid/SecondGrid";
const GridLayout = () => {
  return (
    <div className="flex h-screen w-screen items-center justify-center">
      <div className="grid h-full w-full gap-4 p-10 grid-cols-10 grid-rows-10 ">
        
        <div className="col-span-3 row-span-10 flex items-center justify-center bg-black  box">
            <FirstGrid />
        </div>
        
        <div className="col-span-7 row-span-2 flex items-center justify-center bg-black  box">
            <SecondGrid />
        </div>
        
        <div className="col-span-4 row-span-6 flex items-center justify-center bg-black  box">
        <SecondGrid />

        </div>
        
        <div className="col-span-3 row-span-2 flex items-center justify-center bg-black  box">
        <SecondGrid />

        </div>
        
        <div className="col-span-3 row-span-6 flex items-center justify-center bg-black  box">
        <SecondGrid />

        </div>
        
        <div className="col-span-1 row-span-2 flex items-center justify-center bg-black  box">
        <SecondGrid />

        </div>
        
        <div className="col-span-1 row-span-2 flex items-center justify-center bg-black  box">
        <SecondGrid />

        </div>
        
        <div className="col-span-1 row-span-2 flex items-center justify-center bg-black  box">
        <SecondGrid />

        </div>
        
        <div className="col-span-1 row-span-2 flex items-center justify-center bg-black  box">
        <SecondGrid />

            
        </div>
      </div>
    </div>
  );
};

export default GridLayout;
